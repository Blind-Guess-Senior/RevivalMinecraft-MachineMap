...
3. **维度切换保留平移**：`_panX/_panZ` 不重置，利用 `DIM_SCALE`（地狱 ×8）使地狱门在不同维度的屏幕位置一致
4. **Canvas 字体**：必须用 `this._font` 别直接写死字体名，否则不匹配 CSS 字体栈
5. **散装机器**：yaml 放在 `machines/` 根目录（不放入任何子文件夹），JS 加载时 `_folder = null`。渲染表现：
   - 区域模式（缩小）：显示为独立 emoji 标点，不归入任何区域圆圈
   - 机器模式（放大）：与其他机器一样显示为 emoji + 名称标签
   - 可关联散装站点（站点不设 `region` 字段），散装站点按实际坐标绘制，线路可正常连接
