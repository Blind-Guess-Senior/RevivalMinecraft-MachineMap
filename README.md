...
不适合凑成区域的独立设施，直接把 yaml 放在 `machines/` 根目录（不放任何子文件夹）。
...
| | 区域设施 | 散装设施 |
|---|---|---|
| yaml 位置 | 子文件夹内 | `machines/` 根目录 |
| 地图缩小时 | 合并为一个区域圆圈 | 始终显示为独立 emoji |
| 地图放大后 | 各自显示为独立 emoji | 各自显示为独立 emoji |

**散装设施 + 站点：** 可以建一个同样散装的站点（不填 `region` 字段），让散装设施关联它：
...
| -------- | ---- | --------------------------------------------- |
| `name`   |      | 线路名（多站线路建议填写）                    |
| `stops`  | *    | 站点名数组，多站线路用，如 `[A站, B站, C站]`  |
| `from`   | *    | 起点站名（点对点线路）                        |
| `to`     | *    | 终点站名（点对点线路）                        |
| `method` | ✓    | `矿车` `地狱矿车` `马道` `冰道` `水路` `空路` |
| `time`   |      | 耗时                                          |
